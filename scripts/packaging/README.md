...
runs direct-in with no accounts to create. State lives under
`~/.local/share/datagripe` (`~/Library/Application Support/DataGripe` on
macOS, `%LOCALAPPDATA%\DataGripe` on Windows), so it is still there next
time wherever you run the command from.
